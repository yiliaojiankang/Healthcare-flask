...
deles = {
    '。', '！', '？', '的', '“', '”', '（', '）', ' ', '》', '《', '，', '检查', '考虑',
    '较前', '结合', '提示', '相仿', '多发', '术后', '临床', '新增', '左侧', '右侧', '双侧', '转移', '随诊',
    '复查', '明显', '异常', '未见', '改变'
}


def keys(article, length=100, dele=deles):
    """
    :param dele:
    ...
    try:
        k = list(data['key'])[len(list(data['key'])) / 5: length + len(list(data['key'])) / 5]
    except:
        k = list(data['key'])
    return k


def weights(article, length=100, dele=deles):
    """
    :param article: 需要切分的文本
    :param length: 关键词数量
    :return:
    ...
    data = data.sort_values(by=['value'], ascending=False)
    try:
        w = list(data['value'])[len(list(data['value'])) / 5: length + len(list(data['value'])) / 5]
    except:
        w = list(data['value'])
    return w
